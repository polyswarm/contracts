...
contract('Offer Channel MultiSig', function(accounts) {
  ...
  it("party B starts settle game with old state", async () => {
    ...
    msig.startSettle(s1marshall, sigV, sigR, sigS, { from: expert })
  })
  ...
  it("should revert if already in settlement state", async () => {
    ...
    try {
      await msig.startSettle(s1marshall, sigV, sigR, sigS, { from: expert })
    } catch (err) {
  ...
  it("party A challenges with new state agreed on earlier", async () => {
    ...
    msig.challengeSettle(s2marshall, sigV, sigR, sigS, { from: ambassador })
  })
  ...
  it("should revert if tring to before reply timeout", async () => {
    ...
    try {
      await msig.closeAgreementWithTimeout(s2marshall, sigV, sigR, sigS, { from: ambassador });
    } catch (err) {
      errorMessage = err.message;
    }
